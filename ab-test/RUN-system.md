# A/B run — SYSTEM arm (with the orchestration kit)

**How to use:** open a *fresh* `claude` session (your normal, kitted one) and say:
`Execute the instructions in <the path to this file>`.
The session will do the task below using the kit's workflow, then write its scorecard.

> Run the VANILLA arm (`RUN-vanilla.md`) in a separate fresh chat, then `COMPARE.md` in a third.
...
2. Plan the change, then **dispatch the bounded implementation to implementer sub-agents** (Haiku for
   single-file mechanical, Sonnet for multi-file) with an explicit strict contract: file list, exact change,
   and the verification command (`pytest -q`).
3. Reach for skills that fit (`tdd` for the tests, `review-diff` / `/code-review` for the check).
4. **Review the returned work yourself** before declaring done — verify acceptance, never take a sub-agent's
   word for it.
...
When done, fill this in, **print it in the chat**, AND write it to a file named `result-system.md` in the

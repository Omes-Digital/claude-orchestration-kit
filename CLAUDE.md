...
- **Optional live meter.** Enable the kit's status line (`scripts/statusline.sh` / `statusline.ps1`): it shows `ctx NN% · Nk` and turns yellow with a `/compact` nudge. The nudge fires on **absolute context tokens** (`KIT_COMPACT_TOKENS`, default ~80k — roughly where context rot starts to bite) **or** window % (`KIT_COMPACT_AT`, default 40), whichever hits first. It's token-first on purpose: % is window-relative, so 40% of a 1M-context model is ~400k tokens — far past the useful-context zone — while 80k absolute is "getting chunky" on any window. Opt-in — see `INSTALL.md` §2.

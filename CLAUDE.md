...
- **Optional live meter.** Enable the kit's status line (`scripts/statusline.sh` / `statusline.ps1`): it shows `ctx NN% · Nk` and turns yellow with a `/compact` nudge. Defaults are **model-aware** — the Opus architect is kept leaner than the cheaper implementer tiers (Opus ~80k / 40%; other models ~120k / 60%) — and fire on **absolute tokens** *or* window %, whichever first. Token-first on purpose: % is window-relative, so 40% of a 1M-context model is ~400k tokens (far past the useful zone) while ~80k absolute is "getting chunky" on any window. Override for all models with `KIT_COMPACT_TOKENS` / `KIT_COMPACT_AT`. Opt-in — see `INSTALL.md` §2.
